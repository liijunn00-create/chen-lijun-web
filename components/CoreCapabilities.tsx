        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {capabilityCards.map((capability, index) => (
            <Reveal
              ...
              className="liquid-clear-panel group flex min-h-[330px] flex-col justify-between overflow-hidden p-0 transition hover:-translate-y-1 hover:border-[#9b84b4]/40"
              delay={index * 0.05}
            >
              <div className="p-6 sm:p-7">
                <div className="mb-6 flex items-center justify-between gap-4">
                  <span className="h-2 w-10 bg-[#9b84b4]/70 transition group-hover:w-16" />
                  <span className="text-[11px] font-bold uppercase tracking-[0.22em] text-[#7a86a1]">
                    <TextPressure text="Capability" />
                  </span>
                </div>
                <h3 className="single-line-title text-[clamp(1.18rem,1.55vw,1.55rem)] font-black leading-tight text-[#171513]">
                  <TextPressure text={capability.title} />
                </h3>
                <p className="mt-5 text-base leading-7 text-[#4d4842]">{capability.summary}</p>

                <div className="mt-8 flex flex-wrap gap-2">
                  {capability.proof.map((item) => (
                    <span key={item} className="liquid-clear-chip px-3 py-2 text-sm text-[#342e33]">
                      <TextPressure text={item} />
                    </span>
                  ))}
                </div>
              </div>
              <div className="border-t border-white/25 bg-white/[0.035] p-5">
