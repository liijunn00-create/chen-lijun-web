      ...
      <div className="section-grid">
        ...
        <div className="grid gap-6 xl:grid-cols-3">
          {experienceCases.map((item, index) => (
            <Reveal key={item.title} className="liquid-clear-panel p-6 sm:p-8" delay={index * 0.06}>
              <div className="flex min-h-full flex-col">
                <p className="text-xs uppercase tracking-[0.18em] text-[#7a86a1]">
                  <TextPressure text={item.org} />
                </p>
                ...
                <div className="mt-7 border-t border-white/25 pt-6">
                  <p className="mb-4 text-xs uppercase tracking-[0.18em] text-[#8a847b]">
                    <TextPressure text="关键动作" />
                  </p>
                  <div className="grid gap-3">
                    {item.actions.map((action) => (
                      <span key={action} className="text-sm leading-6 text-[#4d4842]">
                        <TextPressure text={action} />
                      </span>
                    ))}
                  </div>
                </div>
                ...
                <div className="mt-7 grid gap-2 sm:grid-cols-2">
                  {item.results.map((result) => (
                    <span key={result} className="liquid-clear-chip px-3 py-3 text-sm font-semibold text-[#342e33]">
                      <TextPressure text={result} />
                    </span>
                  ))}
                </div>
              </div>
