...
export function Skills() {
  return (
    <section id="ai-skills" className="section-shell">
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {aiCapabilities.map((capability, index) => (
            <Reveal key={capability.title} delay={index * 0.05}>
              <article className="glass-panel group flex min-h-[430px] flex-col justify-between overflow-hidden p-7 transition hover:bg-[#ffe6ef]/45">
                <div>
                ...
                <div className="mt-8 grid grid-cols-2 gap-2">
                  {capability.outputs.map((output) => (
                    <span key={output} className="glass-chip px-3 py-2 text-sm font-semibold text-[#171513]">
                      {output}
                    </span>
                  ))}
                </div>
